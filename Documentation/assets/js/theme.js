...
$('.smooth-scroll,.idocs-navigation a').on('click', function() {
	...
	$('html, body').stop().animate({
      scrollTop: $(sectionTo).offset().top - 120}, 1000, 'easeInOutExpo');
			history.pushState(null, null, sectionTo);
});
...
$(function () {
	if (window.location.hash != '') {
		setTimeout(function () {
			$('html, body').stop().animate({
		      scrollTop: $(window.location.hash).offset().top - 120}, 1000, 'easeInOutExpo');
		}, 1000)
	}

	$('section h4, section h3, section h2').on('click', function(event) {
		  var hashID = $(this).parent().attr('id');
			var url = 'https://docs.deepsoundscript.com/#' + hashID;
